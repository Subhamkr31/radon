...
const getbookbyId = async function(req,res){
    ...
    const getbook = await BookModel.find({author_id:data.author_id}).select({name:1, _id:0})
...
const authName = async function(req,res){
    const saveData = await BookModel.find({ratings:{ $gt : 4}}).select({author_id:1, _id:0})
    const data = await AuthorModel.find({ $or:[{age : {$gt :50}},{author_id:saveData.author_id}]}).select({age:1,author_name:1, _id:0})
